...
def vary_los(prop_sdec, beds, demand, runs,
                 escalation, reneging, prioritisation):
    ...
    747.4578727809351]
    # lists to loop over - adjust as required
    demand_list = [demand] * len(mean_time_in_bed_list)
    sdec_list=[round(x * prop_sdec) for x in demand_list] #set to 0 for all demand through ED
    ed_list=[d - s for d, s in zip(demand_list, sdec_list)]
    #check_list=[e + s for e, s in zip(ed_list, sdec_list)]
    ...
    for i in range(len(mean_time_in_bed_list)):
        #print(demand_list[i])
        ...
        g.other_inter_visit = 0
        g.number_of_nelbeds = beds
        g.mean_time_in_bed = (mean_time_in_bed_list[i] * 60) # convert hrs to minutes
        g.sd_time_in_bed = (sd_time_in_bed_list[i] * 60) # convert hrs to minutes
        #g.sim_duration = (240 * 24 * 60) # convert days into minutes
        #g.warm_up_period = (300 * 24 * 60)
        g.number_of_runs = runs
        g.reneging = reneging
    ...
    df['Demand'] = demand_list
    df['los_hrs'] = mean_time_in_bed_list
    df['Daily 12hr DTAs'] = dtas_list
    df['ED Admissions'] = edadmissions_list
    df['Reneged (per day)'] = reneged_list
    df['Mean DTA Wait'] = meanwait_list
    ...
    #Reshape the DataFrame to long format
    df_long = df.melt(id_vars='los_hrs', 
                    value_vars=value_vars,
                    var_name='Metric', 
                    value_name='Value')
    
    fig = px.line(df_long, x='los_hrs', y='Value', color='Metric', markers=True,
            line_dash='Metric',
            title='LosHrs vs Multiple Metrics',
            labels={'Value': 'Value', 'los_hrs': 'los_hrs'})
...
def find_max_los(df, metric):

    # Find the index where 'Daily 12hr DTAs' first exceeds 0
    idx = df[df[metric] > 0].index.min()

    # Get the Demand just before the threshold breach
    max_los = df.loc[idx - 1, 'los_hrs'] if idx and idx > 0 else None
