...
from distribution_functions import calc_mode, hist_compare_real_model, make_lognormal_lists, visualise_lognormal_hist_list
```
...
mean=csv['LoSHrs'].mean()
std=csv['LoSHrs'].std()
mode=calc_mode(csv, 'LoSHrs', 1, 0)
median=csv['LoSHrs'].median()
perc_25=csv['LoSHrs'].quantile(0.25)
perc_75=csv['LoSHrs'].quantile(0.75)
perc_95=csv['LoSHrs'].quantile(0.95)

summary_df['Metric'] = ['mean','std','mode','25 perc','median','75 perc','95 perc']
summary_df['Real data'] = [mean,std,mode,perc_25, median,perc_75,perc_95]
...
fig_list=hist_compare_real_model("../data/los_fy2425.csv", mean, std)
fig=fig_list[0]
...
mode=16
mean_list, std_list = make_lognormal_lists(mode, 20)
...
visualise_lognormal_hist_list(mean_list1, std_list1, 70000)
```
