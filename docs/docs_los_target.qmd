...
def to_rgba_string(color_name, alpha):
    """Converts a named color and alpha value to an rgba string."""
    # Get the 'rgb(r,g,b)' string
    rgb_string = plotly.colors.convert_colors_to_same_type([color_name])[0][0]
    # Reformat to 'rgba(r,g,b,a)'
    return f"rgba{rgb_string[3:-1]}, {alpha})"
```
...
summary_nctr1.round(1)
```

@tbl-nctr-target-comparison shows how the selected NCTR reduction compares to the modelled target.
