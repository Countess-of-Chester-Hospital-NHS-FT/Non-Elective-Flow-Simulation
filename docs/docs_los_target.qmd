...
# calculate real
fig_list, list_of_summary_lists = distribution_functions.hist_compare_real_model(df_sql['LoSHrs'], "Current LoS Hrs", mean_list, std_list, random_seed=5)
...
Here is a table of our current NCTR metrics and what the targets for the metrics would be given a `{python} min_column` reduction. These can be used as targets for NCTR performance monitoring.
